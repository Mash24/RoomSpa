...
export function generateStaticParams() {
  return getServiceLocationParams().filter(({ slug }) => {
    ...
    return service && service.category !== "sensual";
  });
}
...
export default async function WellnessServiceLocationPage({ params }: PageProps) {
  const { slug, location } = await params;
  return <ServiceLocationContent slug={slug} locationSlug={location} expectedTier="wellness" />;
}
